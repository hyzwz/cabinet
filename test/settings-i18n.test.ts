...
test("getMessage returns localized settings copy for covered demo surfaces", () => {
  ...
  assert.equal(getMessage("settings.integrations.description", "zh"), "配置 agents 可用的工具服务器。启用服务器并提供 API 凭证，以便 agents 访问外部服务。");
  assert.equal(getMessage("settings.notifications.description", "zh"), "配置当 agents 需要你关注时如何接收提醒。");
